#include<curses.h>
#include<stdlib.h>
#include<string.h>
#include<sys/param.h>
char*b,**q;
int(*r)()=refresh,f,c,h=1,y=1,x=0,Y,X=0,S=sizeof(q);
#define P(m)(puts(m),abort(),1);
#define R realloc
#define A(s)calloc(1,s)
#define M memcpy
#define C strcpy
#define L strlen
#define B break;
#define U getmaxyx(stdscr,Y,X);X-=2;for(f=1;f<h;f++)if(L(v[f])>X){c=v[f][X-1];v[f][X-1]=0;mvprintw(f-1,0,v[f]);v[f][X-1]=c;}else mvprintw(f-1,0,v[f]);move(y-1,x);r();
main(i,v)char**v;{if(i^2)P("no file")strcmp(v[1],"-h")||P("^x to save")
f=open(b=v[1],0);f||P("can't read file")
v=              A(  S*2);v[0  ]=b;        while(read(
f,              &c  ,1))if(c  ==1          +9)v=R(
v,S*(2+(  ++h))),v  [h]=A(1)  ,X  =0;else(  v[h]
=R(v[h],  ++X+1)),  v[h][X-1  ]=  c,v[h][X  ]=0;
close(f); initscr(  );noecho  ()  ;keypad(  stdscr,
1);U;for  (;1+1;){  ;switch(  c=  getch(),  erase
(),r(),c  ){case 8  +8    +8  :;  goto Q;;  case 260:
x=x?x-1:  y-1?--y,  L      (  v[  y]):x;B;  case 261:
x=(x-L(v  [y]))?x+     1:     y-  h?++y,0:  x;B
case(259  ):y=MAX(    1,y-    1)  ;x=L(v[y  ])?MIN(
x,L(v[y]  )-1):0;B   case((   258          )):y=
MIN(h,y+  1);x=L(v  [y])?MIN  (x,L        (v[y])-1)
:0;B;case(263):if( !x){if(y>1 ){x=L(v[y-1]);
q=R(0,S*(--h));M(q,v,(y-1)*S);q[y-1]=A(L(v[y])
+L(v[y-1])+5);C(q[y-1],v[y-1]);C(q[y-1]+L(v[y-1]),
v[  y]);M(q+  y,v+y+1,S*(h-y));y
--  ;h--;v=q  ;}}else{b=A(
L(  v[y]));   M(  b,v[y],x-1);C(b+x-1,v[y]+x);v[
y]  =b;x--   ;}B  ;;  case'\n':v=R(v        ,S*(++
h+  1));M   (v+y+2,v  +y+1,(h-y-1)*          S);
v[  y+1]   =A(L(  v[  y])-x+1);C(v  [y+1],v[  y]+x
);  x=v   [y++][  x]  =0;B;default  :if(((((  31+
1)      <=c))&&(  (0  +c<=126)))){  v[y]=R(v  [y],
L(       v[y])+2  );  v[y][L(v[y])  +1]=0;M(  v[y
]+  x+1,  v[y]+x  ,L  (v[y])-x);v[  y][x++]=  c;}B
}U  };Q:   /**GO  TO  SRY -elronnd  */endwin  ();
b=  fopen   (v[0  ],  "w");b||P("  cant write file");
c=  10;for   (Y=  1;  Y<h;Y++){(1)  ;fwrite(  v
[Y  ],L((v[   Y]  ))            ,1,          b);fwrite
(&  c,1,1,b)  ;}  ;;            ;;;;        close(b);}
